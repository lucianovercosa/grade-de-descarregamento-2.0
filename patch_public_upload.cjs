...
const targetFunc = `  if (!vehicleId) return <div className="min-h-screen bg-slate-900 flex items-center justify-center text-white">Veículo não encontrado</div>;`;
const replacementFunc = `  const openAttachment = (e: React.MouseEvent, att: {url: string, type?: string}) => {
  ...
  if (!vehicleId) return <div className="min-h-screen bg-slate-900 flex items-center justify-center text-white">Veículo não encontrado</div>;`;

if (content.includes(targetFunc) && !content.includes('openAttachment')) {
  content = content.replace(targetFunc, replacementFunc);
}

const targetLink = `<a key={idx} href={att.url} target="_blank" rel="noopener noreferrer" className="bg-white border border-slate-200 p-3 rounded text-sm text-blue-600 hover:text-blue-700 hover:underline hover:bg-slate-50 transition-colors flex items-center justify-between font-medium">`;
...
if (content.includes(targetLink)) {
  ...
  console.log('Success Public Upload');
} else {
  console.log('Target not found Public Upload Link');
}
